...
export class CreateActionPlanForStudentUseCase {
    ...
    public async call(req: RequestWithUser, student_id: string, data: CreateActionPlanForStudentDTO) {
        const staff_id = req.user.id;
        const staff_name = req.user.name;
        const student = await this.userService.findUserWithIdAndRole(student_id, Roles.STUDENT);
        if (!student) {
            throw new HttpError(400, 'Student does not exist');
        }

        const actionPlan = await this.actionPlanService.createActionPlanForStudent(student_id, staff_name, staff_id, student.email, data);
        const response = {
            id: actionPlan.id,
            entry_by: staff_name,
