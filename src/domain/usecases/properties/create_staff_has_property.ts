...
export class CreateStaffHasPropertyUseCase {
    ...
    public async call(req: RequestWithUser, staff_id: string, property_id: string) {
        const role = req.user.role;
        if (role === Roles.STUDENT) {
            throw new HttpError(400, 'Student not Authorized');
        };
        const staff_has_property = await this.property_service.addStaffToProperty(property_id, staff_id);
        const result = {
