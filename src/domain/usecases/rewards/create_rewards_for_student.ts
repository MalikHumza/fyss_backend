...
export class CreateRewardsForStudentUseCase {
  ...
  public async call(
    ...
    try {
      ...
      const MAX_CURRENT_POINTS = parseInt(process.env.MAX_CURRENT_POINTS || '500', 10)
      const points = await this.rewardsScoring.getRewardPointsByName(
        data.reward_key,
      );

      const progress = await this.rewardsProgress.getRewardsProgressByStudentId(student_id);
      if (!progress) {
        const rewards = await this.createStudentRewards(staff_id, staff_name, student_id, points.value, data);
        const points_to_next = MAX_CURRENT_POINTS - rewards.points;
        await this.rewardsProgress.createStudentRewardsProgress(student_id, rewards.points, 0, points_to_next, rewards.points);
        const response = this.buildRewardsResponse(rewards, staff_id, staff_name, student_id);
      ...
      const reward_points = await this.rewardService.getStudentRewards(student_id, data.student_email);
      ...
      await this.rewardsProgress.updateStudentRewardsProgress(progress.id, student_id, current_points, new_level, points_to_next, sum_of_points);
  ...
  private async createStudentRewards(staff_id: string, staff_name: string, student_id: string, value: number, data: CreateStudentRewardsDTO) {
  ...
  private buildRewardsResponse(rewards: any, staff_id: string, staff_name: string, student_id: string) {
    return {
      id: rewards.id,
      staff_id,
      staff_name,
