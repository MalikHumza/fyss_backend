...
export class CreateRewardsForStudentUseCase {
    ...
    public async call(req: RequestWithUser, student_id: string, data: CreateStudentRewardsDTO) {
        const staff_id = req.user.id;
        const staff_name = req.user.name;

        const points = await this.rewardsScoring.getRewardPointsByName(data.reward_key);
        const rewards = await this.rewardService.createRewardsForStudent(staff_id, staff_name, student_id, points.value, data);
        const response = {
            id: rewards.id,
            staff_id,
            staff_name,
