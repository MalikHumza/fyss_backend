    ...
    public async call(
        ...
        switch (type) {
            case SAVING_TYPES.DEPOSITOR:
                ...
                const saving_data = await this.savingService.getSavingsByStudentId(student_id, data.student_email);
                if (saving_data.length > 0) {
                    const saving_balance = saving_data[0].balance;
                    const current_balance = saving_balance + data.amount;
                    await this.savingService.updateStudentSavingsBalance(savings.id, student_id, current_balance);
            ...
            case SAVING_TYPES.WITHDRAWL:
                const check_savings = await this.savingService.getSavingsByStudentId(student_id, data.student_email);
                if (check_savings.length > 0) {
                    ...
                    const withdrawl = await this.savingService.createSavingsForStudent(student_id, staff_id, staff_name, data);
                    const available_balance = saving_amount - data.amount;
                    await this.savingService.updateStudentSavingsBalance(withdrawl.id, student_id, available_balance);
                    return new HttpResponse(true, false);
                }
                if (check_savings!.length > 0) {
                    throw new HttpError(400, 'Savings not found');
