...
export class CreateShiftRecordsByPropertyUseCase {
    ...
    public async call(req: RequestWithUser, property_id: string, data: CreateShiftRecordsDTO) {
        const user_id = req.user.id;
        const role = req.user.role;
        if (role === Roles.STUDENT) {
            throw new HttpError(400, 'Student not Authorized');
        }
        const shift = await this.shift_tracker_service.createShiftRecordsByProperty(user_id, property_id, data);
        const response = {
            id: shift.id,
