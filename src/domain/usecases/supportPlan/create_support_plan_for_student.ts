...
export class CreateSupportPlanForStudentUseCase {
    ...
    public async call(req: RequestWithUser, student_id: string, data: CreateSupportPlanDTO) {
        const staff_id = req.user.id;
        const result = await this.supportPlanService.createSupportPlanForStudent(staff_id, student_id, data);
        const response = {
            id: result.id,
            staff_id,
            student_id,
