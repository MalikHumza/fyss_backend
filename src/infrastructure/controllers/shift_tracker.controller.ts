...
import { Authorized, Get, HttpCode, JsonController, Param, Req, UseBefore } from "routing-controllers";
...
export class ShiftTrackerController {
    private getShiftTrackerByPropertyIdUseCase = Container.get(GetShiftTrackerByPropertyIdUseCase);

    @Get('/:property_id')
    @HttpCode(200)
    getAllShiftRecordsyPropertyId(@Req() req: RequestWithUser, @Param('property_id') property_id: string) {
