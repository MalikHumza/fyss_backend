...
import { Authorized, Get, HttpCode, JsonController, QueryParam, Req, UseBefore } from "routing-controllers";
...
export class SupportPlanController {
    private getStudentSupportPlanUseCase = Container.get(GetStudentSupportPlanUseCase);

    @Get('/student')
    @HttpCode(200)
    getStudentSupportPlan(@Req() req: RequestWithUser, @QueryParam('month', { required: true }) months: QUARTER_MONTHS, @QueryParam('year', { required: true }) year: number) {
